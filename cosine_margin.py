...
class CosineMarginFunction(Function):
  ...
  def forward(self, cosine_logits, labels):
    ...
    self.intermediate = masked_cosine_logits, float_labels_onehot, sign_vals
  ...
  def backward(self, grad_margin_cosine_logits):
    labels = self.constant

    batch_size, num_classes = self._cosine_logits_size
    masked_cosine_logits, float_labels_onehot, sign_vals = self.intermediate
...
if __name__ == "__main__":
    ...
    logits = Variable(torch.randn(batch_size, num_classes).float(), requires_grad=True)
